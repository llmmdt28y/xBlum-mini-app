...
export type AppContextType = AppState & {
  ...
  setPersonalizeMemories:  (v: boolean) => void
  t:                       (key: string) => string
  minutesUntilReset:       number
  openStarsPayment:        (invoiceUrl: string) => void
}
...
const RU: TDict = {
  howCanIHelp:"Чем могу помочь сегодня?", typeMessage:"Спросите что угодно...",
  ...
  dailyTokens:"Ежедневная награда", addToChat:"Добавить в чат",
...
const ES: TDict = {
  howCanIHelp:"¿En qué puedo ayudarte hoy?", typeMessage:"Pregunta lo que sea...",
  ...
  dailyTokens:"Recompensa diaria", addToChat:"Añadir al chat",
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setState(s => ({
        ...
        personalizeMemories: saved.personalizeMemories !== undefined ? saved.personalizeMemories : true,
      }))
    } catch {
  ...
  useEffect(() => {
    try {
      localStorage.setItem("xblum-v3", JSON.stringify({
        ...
        personalizeMemories: state.personalizeMemories,
      }))
    } catch {}
  }, [
    state.language, state.userPreferences, state.selectedModel,
    state.tokens, state.isPremium, state.missionTokensToday,
    state.missionResetDate, state.referralCode, state.personalizeMemories,
  ])
  ...
  function openStarsPayment(url: string) {
    const tg = getTg()
    if (!tg) return
    tg.openInvoice(url, (status) => {
      if (status === "paid") {
        setState(s => ({ ...s, isPremium: true }))
        tg.showAlert("✅ Payment successful! You are now Pro.")
      }
    })
  }
  ...
  const value: AppContextType = {
    ...
    openStarsPayment,
    setPersonalizeMemories: (v: boolean) => {
      setState(s => ({ ...s, personalizeMemories: v }))
      try {
        getTg()?.sendData(JSON.stringify({ action: "set_personalize_memories", enabled: v }))
      } catch {}
    },
  }
