...
const AUCTION_ITEMS = [
  { 
    id: 'pepe', 
    title: 'Snake Gift Box', 
    tag: '#2025', 
    collection: 'Special Gifts',
    imgSrc: '/1000010846.jpg', 
    estValue: '100,000', 
    fiatValue: '$6,210.52',
    attributes: [
  ...
  { 
    ...
    title: 'Slurp Ramen', 
    tag: '#1024', 
    collection: 'Foodie Collectibles',
    imgSrc: '/1000010845.jpg', 
    estValue: '1.8', 
    fiatValue: '$12.40',
    attributes: [
  ...
  { 
    id: 'bunny2', 
    title: 'Ahoy Ice Cream', 
    tag: '#8821', 
    collection: 'Foodie Collectibles',
    imgSrc: '/1000010844.jpg', 
    estValue: '0.5', 
    fiatValue: '$3.50',
    attributes: [
...
export function MarketView() {
  ...
  // ── COMPONENTE TOP-UP PILL DINÁMICO ──
    ...
    <div className="flex-1 flex flex-col h-full bg-[#0a0a0b] relative overflow-hidden" ref={dropdownRef}>
      ...
      <div className="flex-1 overflow-y-auto relative z-10 w-full h-full pb-32">
        ...
        {viewingBoxId && activeBoxData ? (
          <div className="animate-in slide-in-from-right-8 fade-in duration-300 min-h-[calc(var(--vh, 1dvh) * 100)] pb-20 pt-8 px-5">
                   ...
                   <div className="absolute left-1/2 top-[24px] -translate-x-1/2 z-[60] w-0 h-0 border-l-[12px] border-l-transparent border-r-[12px] border-r-transparent border-t-[14px] border-t-white" />
                    ...
                    {openingState === 'idle' ? (
                      <div className="w-full h-[160px] relative flex justify-center items-center overflow-hidden animate-in fade-in duration-500 rounded-[24px]">
                        ...
                        <div className="absolute z-20 w-[95px] h-[95px] bg-[#0d0d0f] rounded-[24px] -translate-x-[115px] flex items-center justify-center border border-[#262626] shadow-xl opacity-70">
               ...
               <div className="w-full flex flex-col px-5 border-t border-[#1c1c1e] pt-6">
                  <div className="flex items-center justify-between mb-4">
                     ...
                     <div className="flex gap-2">
                         <button type="button" onClick={() => setBoxViewMode(v => v === 'grid' ? 'list' : 'grid')} className="w-[38px] h-[38px] bg-[#1c1c1e] rounded-[12px] flex items-center justify-center text-white border border-[#262626] active:scale-95 transition-transform shadow-sm">
                           {boxViewMode === 'grid' ? <List className="w-4 h-4" /> : <LayoutGrid className="w-4 h-4" />}
                        </button>
                        <button type="button" className="w-[38px] h-[38px] bg-[#1c1c1e] rounded-[12px] flex items-center justify-center text-[#8e8e93] border border-[#262626] active:scale-95 transition-transform">
                           <ArrowDownUp className="w-4 h-4" />
                        </button>
                     </div>
                  </div>

                  <div className="w-full bg-[#1c1c1e] rounded-[14px] flex items-center px-4 py-2.5 gap-2 border border-[#262626] mb-5">
                     ...
                     <div className="flex flex-col gap-2.5 pb-8">
                        {INSIDE_ITEMS.map((item) => (
                          <div key={item.id} className="w-full bg-[#111111] border border-[#1c1c1e] rounded-[14px] p-1.5 flex items-center gap-3 shadow-sm hover:bg-[#161618] transition-colors cursor-pointer">
                              <div className="w-10 h-10 rounded-[10px] bg-[#1c1c1e] flex items-center justify-center shrink-0 border border-[#262626]">
          ...
          <div className="animate-in slide-in-from-right-8 fade-in duration-300 pb-10 pt-20 flex flex-col gap-6">
            ...
            <div className="flex gap-3 mx-5 mb-2 px-1">
               ...
               <button type="button" onClick={() => setIsMakeOfferOpen(true)} className="flex-1 bg-[#1c1c1e] hover:bg-[#262626] text-[#3b82f6] font-bold text-[15px] py-3.5 rounded-[16px] flex items-center justify-center gap-2 transition-colors border border-[#262626] active:scale-95 shadow-sm" style={{ fontFamily: SF }}>
               ...
               <div className="w-full bg-[#111111] border border-[#1c1c1e] rounded-[24px] p-2 shadow-sm">
                  {activeAuctionData.attributes.map((attr, idx) => (
                     <div key={idx} className={`flex items-center justify-between p-3 ${idx !== activeAuctionData.attributes.length - 1 ? 'border-b border-[#1c1c1e]' : ''}`}>
                        ...
                        <div className="flex items-center gap-1 bg-[#1c1c1e] text-[#8e8e93] text-[12px] font-semibold px-2 py-1 rounded-lg border border-[#262626]">
                 ...
                 <div className="grid grid-cols-2 gap-x-3 gap-y-8 mt-2 pb-6">
                    {MARKET_BOXES.map((box) => (
                       <div key={box.id} onClick={() => setViewingBoxId(box.id)} className="relative flex flex-col group cursor-pointer animate-in fade-in zoom-in-95 duration-300 defer-render">
                          ...
                          {box.id !== 'free' && (
                             <div className="absolute -top-2.5 left-3 z-40 bg-[#0a0a0b] px-2.5 py-[4px] rounded-md border border-white/10 shadow-lg flex items-center gap-1.5 transition-all group-hover:border-white/20">
             ...
             {(activeTab === 'Auctions' || activeTab === 'Listed') && (
                <div className="flex flex-col w-full pt-2 animate-in fade-in slide-in-from-right-4 duration-300">
                  ...
                  <div className="flex gap-2 w-full mb-3 relative">
                     ...
                     <button type="button" className="w-[44px] h-[44px] bg-[#1c1c1e] rounded-[14px] flex items-center justify-center text-[#8e8e93] border border-white/[0.04] active:scale-95 transition-transform shrink-0">
                        <ArrowDownUp className="w-5 h-5" />
                     </button>
                     <button type="button" className="w-[44px] h-[44px] bg-[#1c1c1e] rounded-[14px] flex items-center justify-center text-[#8e8e93] border border-white/[0.04] active:scale-95 transition-transform shrink-0">
                        <Copy className="w-5 h-5" />
                     </button>
                     <button onClick={() => setViewMode(v => v === 'grid' ? 'list' : 'grid')} className="w-[44px] h-[44px] bg-[#1c1c1e] rounded-[14px] flex items-center justify-center text-white border border-white/[0.04] active:scale-95 transition-transform shrink-0 shadow-sm">
                  ...
                  <div className="flex gap-2 w-full mb-4 relative">
                     ...
                     <div className="flex-1 flex gap-2 overflow-x-auto no-scrollbar relative">
                        <div className="relative shrink-0 w-full">
                           <button type="button" onClick={() => setOpenDropdown(openDropdown === 'sale' ? null : 'sale')} className={`w-full h-[44px] bg-[#1c1c1e] rounded-[14px] flex items-center justify-between px-4 font-bold text-[14px] border ${openDropdown === 'sale' ? 'border-[#3b82f6] text-white' : 'border-white/[0.04] text-[#e5e5ea]'} transition-colors`} style={{ fontFamily: SF }}>
                              {filters.sale} <ChevronDown className="w-4 h-4 text-[#8e8e93]" />
                           </button>
                           {openDropdown === 'sale' && (
                              <div className="absolute top-[50px] left-0 bg-[#262626] border border-[#3a3a3c] rounded-[12px] shadow-lg w-full py-1 z-50 animate-in fade-in zoom-in-95 duration-100">
      ...
      {isMakeOfferOpen && activeAuctionData && (
         <div className="fixed inset-0 z-[9999] flex flex-col justify-end">
          ...
          <div className="relative w-full bg-[#161618] rounded-t-[28px] px-5 pt-4 pb-[60px] border-t border-[#262626] flex flex-col max-h-[90vh] overflow-y-auto animate-in slide-in-from-bottom duration-300 transform-gpu">
             ...
             <div className="flex items-center justify-between mb-6">
                ...
                <button type="button" onClick={() => setIsMakeOfferOpen(false)} className="w-8 h-8 rounded-full bg-[#262626] flex items-center justify-center text-white active:scale-95 transition-transform">
                   <X className="w-5 h-5" />
                </button>
             </div>
             ...
             <div className="bg-[#111111] border border-[#1c1c1e] rounded-[20px] p-4 flex flex-col gap-4 mb-4 relative">
                ...
                <div className="bg-[#1c1c1e] rounded-[16px] p-4 flex items-center gap-2 border border-[#262626] relative overflow-hidden w-full">
             ...
             <div className="bg-[#111111] border border-[#1c1c1e] rounded-[20px] p-4 flex flex-col mb-6 border border-[#262626]">
                 <span className="text-white font-bold text-[16px] mb-3" style={{ fontFamily: SFD }}>Offer Duration</span>
                <button type="button" className="w-full bg-[#1c1c1e] border border-[#262626] rounded-[16px] p-4 flex justify-between items-center text-white font-medium text-[15px]" style={{ fontFamily: SF }}>
              ...
              <div className="flex flex-col gap-3 pt-4 border-t border-[#1c1c1e]">
                <div className="flex gap-3">
                   ...
                   <button type="button" onClick={() => { setIsMakeOfferOpen(false); setIsTopUpOpen(true); }} className="w-[56px] h-[56px] bg-[#1c1c1e] border border-[#262626] flex items-center justify-center rounded-[16px] text-white active:scale-95 transition-transform shrink-0">
      ...
      {isAddGiftOpen && (
        <div className="fixed inset-0 z-[9999] flex flex-col justify-end">
          ...
          <div className="relative w-full bg-[#161618] rounded-t-[28px] px-5 pt-4 pb-[60px] border-t border-[#262626] flex flex-col max-h-[85vh] overflow-y-auto animate-in slide-in-from-bottom duration-300 transform-gpu">
