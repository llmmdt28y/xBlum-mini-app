...
const MARKET_BOXES = [
  { id: 'secret', name: 'Secret', color: '#eab308', image: '/1000009369.png', isSoldOut: false },
  { id: 'toxic', name: 'Toxic Whisper', color: '#c084fc', image: '/1000009370.png', isSoldOut: false },
  { id: 'eternal', name: 'Eternal Beacon', color: '#a855f7', image: '/1000009371.png', isSoldOut: true },
  { id: 'aureus', name: 'Aureus', color: '#facc15', image: '/1000009361.png', isSoldOut: false }
]
...
const LootboxVisual = ({ color, imgSrc }: { color: string, imgSrc: string }) => (
  <div className="relative w-full h-[110px] flex flex-col items-center justify-center mb-1 mt-3">
    <div className="absolute bottom-4 w-[60px] h-[60px] opacity-30 rounded-full z-0" style={{ backgroundColor: color, filter: 'blur(20px)' }}></div>
    <div className="relative z-10 animate-box-float">
      <img src={imgSrc} alt="Lootbox" draggable={false} className="w-[90px] h-[90px] object-contain mix-blend-screen pointer-events-none select-none" style={{ WebkitTouchCallout: "none" }} />
    </div>
  </div>
)
...
export function MarketView() {
  ...
  // ── Lógica y Formateo del Input ──
  const handleStarInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    // Quitamos las comas para leer el número real
  ...
  // Variables calculadas para validación y UI
          ...
          <div className="mt-10 px-5">
             <div className="flex items-center justify-between mb-5">
                ...
                <div className="flex gap-2">
                   <button className="w-10 h-10 rounded-[14px] bg-[#1c1c1e] flex items-center justify-center active:scale-95 transition-transform"><Filter className="w-[18px] h-[18px] text-[#3b82f6]" strokeWidth={2.5} /></button>
                   <button className="w-10 h-10 rounded-[14px] bg-[#1c1c1e] flex items-center justify-center active:scale-95 transition-transform"><Star className="w-[18px] h-[18px] text-[#8e8e93]" strokeWidth={2.5} /></button>
                </div>
             </div>
                ...
                {MARKET_BOXES.map((box) => (
                   <div key={box.id} className="flex flex-col w-full">
                      <div className="w-full bg-[#111111] rounded-[22px] p-2 flex flex-col relative border border-[#1c1c1e]">
                         ...
                         <button className="w-full bg-[#2c2c2e] text-white font-bold text-[13px] py-2 rounded-[14px] mt-1 active:scale-95 transition-transform" style={{ fontFamily: SF }}>Market</button>
                      </div>
        ...
        <div className="fixed inset-0 z-[9999] flex flex-col justify-end pointer-events-auto">
          ...
          <div className="relative w-full bg-[#000000] rounded-t-[28px] px-5 pt-4 pb-[60px] border-t border-[#1c1c1e] flex flex-col max-h-[85vh] overflow-y-auto animate-in slide-in-from-bottom duration-300 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
            ...
            <div className="flex flex-col items-center mb-8">
               ...
               {/* Input Centrado Dinámicamente con Animación de Error */}
               <div className={`flex items-center justify-center gap-3 ${isError ? 'animate-shake' : ''}`}>
                 ...
                 <input 
                    ...
                    style={{ 
                       fontFamily: SFD,
                       width: displayValue ? `${displayValue.length}ch` : '1.2ch'
                    }}
                    className={`bg-transparent font-bold text-[56px] min-w-[32px] outline-none placeholder:text-[#3a3a3c] caret-[#3b82f6] transition-colors ${
                       isError ? 'text-[#ff3b30]' : 'text-white'
                    }`}
                 />
               </div>

               {/* Texto de Ayuda / Límite */}
               <span className={`text-[13px] mt-2 font-medium transition-colors ${isError ? 'text-[#ff3b30]' : 'text-[#636366]'}`} style={{ fontFamily: SF }}>
                  Buy between 15 and 150,000 stars
               </span>

               {/* Botón de Recarga Validado */}
               <button 
                  disabled={!isValid}
                  className={`mt-5 w-full max-w-[300px] py-3.5 rounded-[14px] font-bold text-[17px] transition-all duration-300 active:scale-95 ${
                     isValid 
                  ...
                  style={{ fontFamily: SF }}
               >
                  Buy {displayValue ? displayValue : '0'} Stars
               </button>
            </div>
                      ...
                      <div className="relative flex items-center" style={{ width: `${22 + (pkg.count - 1) * 4.5}px`, height: '22px' }}>
                        {Array.from({ length: pkg.count }).map((_, idx) => (
                           <img 
                             ...
                             style={{ left: `${idx * 4.5}px`, zIndex: 20 - idx, filter: "drop-shadow(1.5px 0px 0px #000000)" }}
                             alt="star"
                           />
                        ))}
                      </div>
