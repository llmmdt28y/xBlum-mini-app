...
const ModalInfoRow = ({ label, children, isLast }: any) => (
  <div className={`flex items-center gap-2 py-2 px-3 ${!isLast ? 'border-b border-[#2c2c2e]/50' : ''}`}>
    ...
    <div className="w-[1px] h-3.5 bg-[#2c2c2e] shrink-0" />
    <div className="text-white font-medium text-[13px] flex-1 text-left flex items-center" style={{ fontFamily: SF }}>
      {children}
    </div>
  </div>
)
...
export function ProfileView() {
  ...
  return (
    <div className="flex-1 overflow-y-auto relative animate-in fade-in duration-300 scrollbar-native" style={{ background: "#000000" }}>
        ...
        <div className="w-full flex justify-center z-10 relative mb-4 mt-8">
          <div className="flex items-center bg-[#1c1c1e] rounded-full p-[3px]">
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-[14px] font-medium transition-all ${activeMainTab === "inventory" ? "bg-[#1f2b3e] text-[#60a5fa] shadow-sm" : "text-[#8e8e93] hover:text-white bg-transparent"}`}
              style={{ fontFamily: SF }}
            >
              inventory
            </button>
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-[14px] font-medium flex items-center gap-1.5 transition-all ${activeMainTab === "gifts" ? "bg-[#1f2b3e] text-[#60a5fa] shadow-sm" : "text-[#8e8e93] hover:text-white bg-transparent"}`}
              style={{ fontFamily: SF }}
            >
            ...
            <div className="w-full flex flex-col">
              <div className="flex items-center justify-center gap-2 mb-4 mt-1">
                <button
                  ...
                  className={`px-3 py-1 rounded-full text-[13px] font-medium transition-colors ${activeInventorySubTab === "cosmetics" ? "bg-[#2c2c2e] text-white" : "text-[#8e8e93] bg-transparent hover:text-white"}`}
                  style={{ fontFamily: SF }}
                >
                  cosmetics
                </button>
                <button
                  ...
                  className={`px-3 py-1 rounded-full text-[13px] font-medium transition-colors ${activeInventorySubTab === "achievements" ? "bg-[#2c2c2e] text-white" : "text-[#8e8e93] bg-transparent hover:text-white"}`}
                  style={{ fontFamily: SF }}
                >
                  achievements
                </button>
            ...
            <div className="w-full flex flex-col relative pb-[80px]">
              <div className="flex items-center justify-center gap-2 mb-4 mt-1">
                <button
                  ...
                  className={`px-3 py-1 rounded-full text-[13px] font-medium transition-colors ${activeGiftsSubTab === "all gifts" ? "bg-[#2c2c2e] text-white" : "text-[#8e8e93] bg-transparent hover:text-white"}`}
                  style={{ fontFamily: SF }}
                >
                  all gifts
                </button>
                <button
                  ...
                  className={`px-3 py-1 rounded-full text-[13px] font-medium transition-colors ${activeGiftsSubTab === "vault" ? "bg-[#2c2c2e] text-white" : "text-[#8e8e93] bg-transparent hover:text-white"}`}
                  style={{ fontFamily: SF }}
                >
                  vault
                </button>
                  ...
                  {userHasGifts ? (
                     ...
                     <div className="flex flex-col items-center justify-center pt-8 pb-10">
                        <div className="relative w-[140px] h-[140px] mb-4">
                           <img src="/empty-gift.gif" alt="No gifts" className="w-full h-full object-contain pointer-events-none select-none" draggable={false} style={{ WebkitTouchCallout: "none", filter: "grayscale(100%) opacity(0.7)" }} />
                        </div>
      ...
      {isStylePickerOpen && (
        <div className="fixed inset-0 z-[999] bg-[#0a0a0b] flex flex-col animate-in slide-in-from-bottom-full duration-300 overflow-hidden page-fija" style={{ height: "var(--tg-viewport-height, 100dvh)", position: "fixed", top: 0, left: 0, bottom: 0, right: 0 }}>
          ...
          <div className="relative w-full h-[320px] shrink-0 flex flex-col items-center justify-center pt-4 border-b border-white/5 bg-[#0a0a0b] z-10">
            <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
              {previewBg && COSMETIC_ITEMS_DB[previewBg]?.getEquipped ? COSMETIC_ITEMS_DB[previewBg].getEquipped() : <div className="w-full h-full bg-[#0a0a0b]" />}
          ...
          <div className="flex-1 min-h-0 bg-[#141415] overflow-y-auto relative z-20 pb-6 scrollbar-native">
            <div className="grid grid-cols-3 gap-3 p-4">
              ...
              {Object.values(COSMETIC_ITEMS_DB).filter(i => i.type === 'Profile Background').map(item => {
                ...
                const isOwned = currentLevel.lv >= item.reqLevel;
                return (
                  <button key={item.id} onClick={() => setPreviewBg(item.id)} className={`relative aspect-square rounded-[20px] overflow-hidden border-[2px] transition-all ${isSelected ? 'border-[#007aff] ring-2 ring-[#007aff]/30' : 'border-white/5'} ${!isOwned ? 'grayscale opacity-60' : ''} bg-[#111111]`}>
