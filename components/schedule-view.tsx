...
interface ScheduleItem {
  id: number;
  title: string; 
  description: string; // El prompt
  repeat_type: string; // "Once", "Daily", "Weekly", "Monthly"
  fire_at: string; 
  extra: string; // JSON almacenando tiempo exacto, switches y dias configurados
}
...
function Toggle({ on, onToggle, disabled, activeColor = "#ffffff" }: { on: boolean; onToggle: () => void; disabled?: boolean; activeColor?: string }) {
  return (
    ...
    >
      <span className="absolute rounded-full transition-all duration-100" style={{ width: "16px", height: "16px", top: "4px", background: on ? "#000000" : "#8e8e93", left: on ? "22px" : "4px" }} />
    </button>
  )
}
...
function Section({ children }: { children: React.ReactNode }) {
  return <div className="rounded-[20px] overflow-hidden bg-[#111111] w-full border border-white/5">{children}</div>
}
...
const ExpandingInput = ({ label, maxLength, value, onChange, placeholder = "", isTextArea = false }: { label: string, maxLength: number, value: string, onChange: (v: string) => void, placeholder?: string, isTextArea?: boolean }) => {
    ...
    <div className="relative w-full shrink-0">
      ...
      {isTextArea ? (
        ...
        <input
          ...
          type="text" value={value} onChange={handleChange} onFocus={() => setIsFocused(true)} onBlur={() => setIsFocused(false)} placeholder={placeholder}
...
export function ScheduleView() {
  ...
  const resetForm = () => {
    ...
    setPushEnabled(true); setEmailEnabled(false);
  }
  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen bg-[#000000] text-white select-none overflow-hidden relative">
      ...
      {/* ── Header Principal ── */}
      <div className="px-5 pt-[calc(var(--tg-safe-area-inset-top,24px)+16px)] pb-4 flex items-center justify-between relative z-10">
      ...
      <div className="px-5 pb-10 flex flex-col gap-4 overflow-y-auto no-scrollbar relative z-10 flex-1">
        {loadingItems ? (
          ...
          tasks.map(t => <GrokTaskCard key={t.id} item={t} />)
        )}
      </div>
        ...
        <div className="absolute inset-0 z-[60] bg-[#000000] flex flex-col animate-in slide-in-from-bottom duration-300">
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar px-5 pb-32 space-y-6 pt-4">
            ...
            <div className="flex flex-col gap-4">
               <div className="relative w-full shrink-0">
                  <label className="absolute -top-2.5 left-3 px-1.5 text-[13px] bg-[#000000] z-10 font-medium text-[#8e8e93]" style={{ fontFamily: SF }}>Frequency</label>
                  <select value={frequency} onChange={e => setFrequency(e.target.value)} className="w-full bg-transparent border-[1.5px] border-[#555558] rounded-[12px] px-4 py-3.5 text-white focus:outline-none appearance-none focus:border-[#ffffff] transition-colors" style={{ fontFamily: SF, fontSize: "16px" }}>
                    <option value="Once" className="bg-[#111111]">Una vez</option>
                    <option value="Daily" className="bg-[#111111]">Diariamente</option>
                    <option value="Weekly" className="bg-[#111111]">Semanalmente</option>
                    <option value="Monthly" className="bg-[#111111]">Mensualmente</option>
                  </select>
               </div>

               {frequency === "Once" && (
                 <div className="relative w-full shrink-0">
                    <label className="absolute -top-2.5 left-3 px-1.5 text-[13px] bg-[#000000] z-10 font-medium text-[#8e8e93]" style={{ fontFamily: SF }}>Date</label>
                    <input type="date" value={specificDate} onChange={e => setSpecificDate(e.target.value)} className="w-full bg-transparent border-[1.5px] border-[#555558] rounded-[12px] px-4 py-3.5 text-white focus:outline-none focus:border-[#ffffff] transition-colors" style={{ fontFamily: SF, fontSize: "16px" }}/>
                 </div>
               )}

               {frequency === "Weekly" && (
                 <div className="relative w-full shrink-0">
                    <label className="absolute -top-2.5 left-3 px-1.5 text-[13px] bg-[#000000] z-10 font-medium text-[#8e8e93]" style={{ fontFamily: SF }}>Day of Week</label>
                    <select value={dayOfWeek} onChange={e => setDayOfWeek(e.target.value)} className="w-full bg-transparent border-[1.5px] border-[#555558] rounded-[12px] px-4 py-3.5 text-white focus:outline-none appearance-none focus:border-[#ffffff] transition-colors" style={{ fontFamily: SF, fontSize: "16px" }}>
                      <option value="Mon" className="bg-[#111111]">Lunes</option>
                      <option value="Tue" className="bg-[#111111]">Martes</option>
                      <option value="Wed" className="bg-[#111111]">Miércoles</option>
                      <option value="Thu" className="bg-[#111111]">Jueves</option>
                      <option value="Fri" className="bg-[#111111]">Viernes</option>
                      <option value="Sat" className="bg-[#111111]">Sábado</option>
                      <option value="Sun" className="bg-[#111111]">Domingo</option>
                    </select>
                 </div>
               )}
               ...
               {frequency === "Monthly" && (
                 <div className="relative w-full shrink-0">
                    <label className="absolute -top-2.5 left-3 px-1.5 text-[13px] bg-[#000000] z-10 font-medium text-[#8e8e93]" style={{ fontFamily: SF }}>Day of Month</label>
                    <input type="number" min="1" max="31" value={dayOfMonth} onChange={e => setDayOfMonth(e.target.value)} className="w-full bg-transparent border-[1.5px] border-[#555558] rounded-[12px] px-4 py-3.5 text-white focus:outline-none focus:border-[#ffffff] transition-colors" style={{ fontFamily: SF, fontSize: "16px" }}/>
                 </div>
               )}

               <div className="relative w-full shrink-0">
                  <label className="absolute -top-2.5 left-3 px-1.5 text-[13px] bg-[#000000] z-10 font-medium text-[#8e8e93]" style={{ fontFamily: SF }}>Time</label>
                  <input type="time" value={time} onChange={e => setTime(e.target.value)} className="w-full bg-transparent border-[1.5px] border-[#555558] rounded-[12px] px-4 py-3.5 text-white focus:outline-none appearance-none focus:border-[#ffffff] transition-colors" style={{ fontFamily: SF, fontSize: "16px" }}/>
               </div>
            </div>
