...
// ── Mock Initial Tasks (Con archivos y descripción) ──
const INITIAL_TASKS = [
    { 
        id: 1, 
    ...
    function handleSaveConfig() {
        ...
        setTimeout(() => {
            setTasks(prev => [{
                ...
                color: taskColor,
                desc: taskDesc,
                files: attachedFiles.map(f => f.name), // Guardamos los nombres para mostrarlos en UI
            ...
            <div className="relative z-10 flex-1 flex flex-col pb-32 overflow-y-auto no-scrollbar">
                ...
                <div className="mt-6 flex flex-col items-center gap-3 px-5">
                    {priorityTasks.length > 0 ? (
                        priorityTasks.map(t => (
                            <div key={t.id} className={`flex items-center justify-between w-full max-w-[340px] bg-[#1c1c1e] px-5 py-3.5 rounded-[20px] border border-[#2c2c2e] ${isEditingMode ? 'jiggle-card' : ''}`}>
                                <div className="flex items-center gap-3">
                                    <div className="[&>svg]:w-4 [&>svg]:h-4" style={{ color: t.color }}>{ICON_MAP[t.iconName] || <CalendarDays />}</div>
                                    <span className="text-[15px] font-medium" style={{ color: t.color, fontFamily: SF }}>{t.title}</span>
                                </div>
                                <span className="text-[14px] opacity-70" style={{ color: t.color }}>{t.time}</span>
                                {isEditingMode && (
                                    <button onClick={() => setTasks(tasks.filter(task => task.id !== t.id))} className="absolute -top-1.5 -right-1.5 w-6 h-6 bg-red-500 rounded-full flex items-center justify-center active:scale-90 transition-transform shadow-xl">
                        ...
                        <>
                            <div className="flex items-center justify-between w-full max-w-[340px] bg-[#1c1c1e] px-5 py-3.5 rounded-[20px] border border-[#2c2c2e]">
                                <div className="flex items-center gap-3">
                                    <Moon className="w-4 h-4 text-[#f59e0b]" />
                                    <span className="text-[#f59e0b] text-[15px] font-medium" style={{ fontFamily: SF }}>Morning grogginess</span>
                                </div>
                                <span className="text-[#f59e0b] opacity-70 text-[14px]">15m left</span>
                            </div>
                            <div className="flex items-center justify-between w-full max-w-[340px] bg-[#1c1c1e] px-5 py-3.5 rounded-[20px] border border-[#2c2c2e]">
                                <div className="flex items-center gap-3">
                                    <TrendingUp className="w-4 h-4 text-[#22c55e]" />
                                    <span className="text-[#22c55e] text-[15px] font-medium" style={{ fontFamily: SF }}>Alertness rise</span>
                                </div>
                                <span className="text-[#22c55e] opacity-70 text-[14px]">in 45m</span>
                            </div>
                        </>
                    )}
                </div>
                ...
                <div className="px-5 mt-10 flex flex-col items-center">
                    {tasks.length === 0 ? (
                        <div className="animate-in fade-in duration-500 w-full max-w-[340px]">
                            ...
                            <div className="flex flex-col gap-3 w-full">
                                {SUGGESTIONS.map((sug, idx) => (
                                    <button 
                                        ...
                                        className="flex items-center justify-between w-full bg-[#1c1c1e] px-5 py-3.5 rounded-[20px] border border-[#2c2c2e] active:scale-[0.98] transition-transform text-left"
                                    >
                                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col gap-3 w-full">
                                {filteredTasks.length === 0 ? (
                                    ...
                                    filteredTasks.map(task => {
                                        ...
                                        return (
                                            <div key={task.id} className={`relative flex flex-col w-full bg-[#1c1c1e] border border-[#2c2c2e] rounded-[20px] transition-all ${isEditingMode ? 'jiggle-card' : ''}`}>
                                                
                                                {/* Header Píldora */}
                                                <button onClick={() => toggleTaskExpansion(task.id)} className="flex items-center justify-between w-full px-5 py-4 active:bg-white/5 transition-colors rounded-[20px]">
                                                    <div className="flex items-center gap-3">
                                                        <div className="[&>svg]:w-4 [&>svg]:h-4" style={{ color: task.color || '#3b82f6' }}>
                                                            {ICON_MAP[task.iconName] || <CalendarDays />}
                                                        </div>
                                                        <span className="text-white text-[15px] font-medium truncate" style={{ fontFamily: SF }}>{task.title}</span>
                                                    </div>
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-[#8e8e93] text-[13px]">{task.time}</span>
                                                        <ChevronDown className={`w-4 h-4 text-[#8e8e93] transition-transform ${isExpanded ? 'rotate-180' : ''}`} />
                                                    </div>
                                                </button>

                                                {/* Contenido Desplegable (Acordeón) */}
                                                {isExpanded && (
                                                    <div className="px-5 pb-5 pt-1 animate-in slide-in-from-top-2 fade-in duration-200 border-t border-[#2c2c2e]">
                                                        
                                                        {/* Status Tag */}
                                                        <div className="flex items-center gap-2 mb-3 mt-3">
                                                            <span className="px-2.5 py-1 rounded-[8px] text-[10px] font-bold uppercase tracking-wider" style={{ background: `${task.color || '#3b82f6'}15`, color: task.color || '#3b82f6' }}>
                                                                {task.status}
                                                            </span>
                                                            <span className="text-[#636366] text-[12px]">{task.iconName}</span>
                                                        </div>
                                                        ...
                                                        {task.files && task.files.length > 0 && (
                                                            <div className="flex flex-col gap-2">
                                                                <p className="text-[#8e8e93] text-[11px] font-bold uppercase tracking-wider mb-0.5">Attachments</p>
                                                                {task.files.map((file: string, idx: number) => (
                                                                    <div key={idx} className="flex items-center gap-2 bg-[#2c2c2e] px-3 py-2 rounded-[10px]">
                                                        ...
                                                        {!task.desc && (!task.files || task.files.length === 0) && (
                                                            <p className="text-[#636366] text-[13px] italic mt-2">No additional details provided.</p>
                                                        )}
                                                    </div>
                                                )}
                            ...
                            <div className="bg-[#1c1c1e] rounded-[28px] divide-y divide-white/5 overflow-hidden">
                                ...
                                {(eventType === "Schedule Email" || eventType === "Drive Upload" || eventType === "Telegram Post" || eventType === "Custom Event") && (
                                    <div className="flex items-center justify-between w-full p-4 border-t border-[#2c2c2e]">
                                        ...
                                        <label htmlFor="file-upload" className="text-blue-500 font-medium truncate max-w-[140px] cursor-pointer bg-[#2c2c2e] px-3 py-1.5 rounded-full text-[13px] active:scale-95 transition-transform text-center">
