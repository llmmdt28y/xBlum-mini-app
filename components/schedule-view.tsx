...
export function ScheduleView() {
    ...
    useEffect(() => {
        ...
        // Muestra el botón si hay algo abierto por encima de la vista principal
    ...
    return (
        <div className="flex-1 flex flex-col min-h-screen bg-[#000000] text-white select-none overflow-hidden relative">
                        ...
                        <div className="space-y-4 animate-in fade-in duration-500">
                            ...
                            {filteredTasks.length === 0 ? (
                                <div className="p-8 text-center bg-[#1c1c1e] rounded-[28px] border border-dashed border-[#2c2c2e]">
                                    <p className="text-[#636366] font-medium">No standard events for this selection</p>
                                </div>
                            ) : (
                                        ...
                                        <div className="bg-[#1c1c1e] rounded-[24px] p-5 flex items-center justify-between shadow-lg">
                                            <div className="flex items-center gap-4">
                                                <div className="w-11 h-11 rounded-full flex items-center justify-center bg-[#000]/40" style={{ border: `1px solid ${task.color}30` }}>
                                                    {ICON_MAP[task.iconName] ? <div style={{ color: task.color }}>{ICON_MAP[task.iconName]}</div> : <CalendarDays className="w-5 h-5" style={{ color: task.color }} />}
                                                </div>
                                                <div>
            ...
            {/* ── MODAL: PANEL DE CONFIGURACIÓN ÚNICO (Estilos Animados) ── */}
            {configModalOpen && (
                        ...
                        <div className="overflow-y-auto flex-1 no-scrollbar pb-8 space-y-4">
                            <div className="bg-[#1c1c1e] rounded-[28px] p-2 shadow-inner">
                                <div className="flex flex-col divide-y divide-[#2c2c2e]">
                                    ...
                                    <div className="flex flex-col">
                                        ...
                                        {activePicker === "type" && (
                                            <div className="flex items-center justify-center py-4 bg-[#0a0a0a] rounded-[20px] my-1 mx-2 origin-top animate-in fade-in zoom-in-95 slide-in-from-top-2 duration-200 ease-out wheel-mask">
                                    ...
                                    <div className="flex flex-col">
                                        <div className="flex items-center justify-between w-full p-4">
                                            ...
                                            <input type="text" placeholder="Add title..." value={taskTitle} onChange={e=>setTaskTitle(e.target.value)} className="bg-transparent text-right text-white w-32 focus:outline-none font-medium" />
                                        </div>
                                        {activePicker === "icon" && (
                                            <div className="grid grid-cols-6 gap-3 py-4 px-4 bg-[#0a0a0a] rounded-[20px] my-1 mx-2 origin-top animate-in fade-in zoom-in-95 slide-in-from-top-2 duration-200 ease-out">
                                    ...
                                    {/* ── Color Theme Row ── */}
                                    <div className="flex flex-col">
                                        <button onClick={() => togglePicker("color")} className="flex items-center justify-between w-full p-4 active:bg-[#2c2c2e] rounded-2xl transition-colors">
                                            <div className="flex items-center gap-3">
                                                <div className="w-[20px] h-[20px] rounded-full" style={{ background: taskColor }} />
                                                <span className="text-white text-[16px] font-medium">Theme Color</span>
                                            </div>
                                            <ChevronRight className={`w-5 h-5 text-[#8e8e93] transition-transform ${activePicker === 'color' ? 'rotate-90' : ''}`} />
                                        </button>
                                        {activePicker === "color" && (
                                            <div className="flex justify-around py-4 px-4 bg-[#0a0a0a] rounded-[20px] my-1 mx-2 origin-top animate-in fade-in zoom-in-95 slide-in-from-top-2 duration-200 ease-out">
                                                {THEME_COLORS.map(c => (
                                    ...
                                    <div className="flex flex-col">
                                        ...
                                        {activePicker === "time" && (
                                            <div className="flex items-center justify-center gap-6 py-4 bg-[#0a0a0a] rounded-[20px] my-1 mx-2 origin-top animate-in fade-in zoom-in-95 slide-in-from-top-2 duration-200 ease-out wheel-mask">
                                    ...
                                    <div className="flex justify-between w-full p-4 items-center">
                                        <div className="flex items-center gap-3">
                                            <Sparkles className="w-[20px] h-[20px] text-[#f59e0b]" />
                                            <span className="text-white text-[16px] font-medium">Set as Priority</span>
                                        </div>
                                        <button 
                                            onClick={() => setIsPriority(!isPriority)} 
                                            className={`w-12 h-7 rounded-full relative transition-colors duration-300 ${isPriority ? 'bg-blue-500' : 'bg-[#3a3a3c]'}`}
                                        >
                                            <div className={`absolute top-1 w-5 h-5 bg-white rounded-full transition-all duration-300 shadow-sm ${isPriority ? 'left-6' : 'left-1'}`} />
                                        </button>
                                    </div>
                                    ...
                                    <div className="flex flex-col">
                                        <button onClick={() => togglePicker("reminder")} className="flex items-center justify-between w-full p-4 active:bg-[#2c2c2e] rounded-2xl transition-colors">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-white text-[16px] font-medium">Alert me</span>
                                            </div>
                                            <span className="text-[#8e8e93] text-[16px]">{selRemMin}m {selRemSec}s before</span>
                                        </button>
                            ...
                            <div className="bg-[#1c1c1e] rounded-[28px] p-4 flex flex-col gap-2">
                                <div className="flex items-center gap-3 pl-2">
                                    ...
                                    <span className="text-white text-[16px] font-medium">Notes & Context</span>
                                </div>
                                <textarea 
                                    rows={3}
                                    placeholder="Optional AI prompt or details..."
                                    value={taskDesc}
