...
export function HomeView() {
  const {
    ...
    isThrottled, minutesUntilReset, userId,
  } = useApp()
  ...
  function sendMessage() {
    ...
    // Enviar el mensaje directamente al bot via deep link ?start=msg_<encoded>
    // El bot detecta el parámetro msg_ y procesa el mensaje inmediatamente,
    // respondiendo con la AI sin modo inline (la respuesta llega citando el texto)
    const encoded = encodeURIComponent(text)
    const tg = window.Telegram?.WebApp
    if (tg) {
      tg.openTelegramLink(`https://t.me/${BOT}?start=msg_${encoded}`)
    }
    setMessage("")
  }
    ...
    <div className="flex-1 flex flex-col items-center justify-center px-4 pb-8 relative">
      ...
      <div className="flex flex-col items-center gap-5 w-full max-w-md relative z-10">
        ...
        <div className="relative">
          <img src="/xblum-logo-blue.png" alt="xBlum"
          ...
          <div className="w-full p-3.5 bg-orange-500/10 border border-orange-500/30 rounded-2xl flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="mt-2 flex gap-3">
                <button onClick={() => setCurrentView("settings")}
                  className="text-xs text-blue-400 underline underline-offset-2">{t("changeModel")}</button>
                {!isPremium && (
                  <button onClick={() => setCurrentView("premium")}
                    className="text-xs text-amber-400 underline underline-offset-2">upgrade Pro</button>
                )}
              </div>
            </div>
          </div>
        ...
        {!isPremium && (
          <button onClick={() => setCurrentView("premium")}
            ...
            <div className="flex items-center gap-3">
              <img src="/xblum-logo-blue.png" alt=""
              ...
              <div>
                ...
                <p className="text-neutral-500 text-xs mt-0.5">1,000 tokens/mo · GPT-5.4 · from 800 ⭐</p>
              </div>
            </div>
          </button>
        )}

        {/* Input — envía directo al bot (no inline) */}
        <div className="w-full">
          ...
          {message.trim() && (
            <p className="text-xs text-neutral-600 mt-1.5 text-center">
              Opens xBlum and sends your message directly
            </p>
          )}
        </div>
