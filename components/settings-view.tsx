...
function Section({ title, footer, children, rightAction }: { title?: string; footer?: React.ReactNode; children: React.ReactNode; rightAction?: React.ReactNode }) {
  return (
    <div className="flex flex-col w-full mb-6"> 
      ...
      <div className="flex flex-col gap-[2px] relative [&>*]:bg-[#111111] [&>*]:rounded-[4px] [&>*:first-child]:rounded-t-[16px] [&>*:last-child]:rounded-b-[16px] [&>*]:overflow-hidden">
        {children}
      </div>
      {footer && (
...
export function SettingsView({ 
  ...
  return (
    ...
    <div key="main" className={`flex-1 ${showReportModal ? 'overflow-hidden' : 'overflow-y-auto overscroll-none'} animate-in fade-in zoom-in-[0.98] duration-200 ease-out`} style={{ background: "#000000", height: "calc(var(--vh, 1dvh) * 100)", maxHeight: "calc(var(--vh, 1dvh) * 100)" }}>
       <style>{RIPPLE_STYLE}</style>
